...
public partial struct CharacterDeathServerSystem : ISystem
{
    ...
    public void OnCreate(ref SystemState state)
    {
        ...
        state.RequireForUpdate(SystemAPI.QueryBuilder().WithAll<Health, FirstPersonCaharacterComponent>().WithDisabled<DelayedDespawn>().Build());
    ...
    public partial struct CharacterDeathServerJob : IJobEntity
    {
        ...
        void Execute(Entity entity, in FirstPersonCaharacterComponent character, in Health health,
...
public partial struct CharacterDeathClientSystem : ISystem
{
    ...
    public void OnCreate(ref SystemState state)
    {
        state.RequireForUpdate(SystemAPI.QueryBuilder().WithAll<FirstPersonCaharacterComponent, DelayedDespawn>().Build());
    }
    ...
    [BurstCompile]
    [WithAll(typeof(FirstPersonCaharacterComponent), typeof(DelayedDespawn))]
    public partial struct CharacterDeathClientJob : IJobEntity
    {
        ...
        void Execute(ref FirstPersonCaharacterComponent character, ref VfxAttributeSettings vfxAttributeSettings)
        {
                ...
                if (LocalToWorldLookup.TryGetComponent(character.DeathVfxSpawnPoint, out LocalToWorld deathVfxLtW))
                {
                    ...
                    Ecb.AddComponent(spawnVfxDeathRequestEntity,
                        new VfxHitRequest()
                        {
                            ...
                            HighCunt = vfxAttributeSettings.HighVfxSpawnCount,
