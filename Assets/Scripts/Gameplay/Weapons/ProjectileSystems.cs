...
public partial class ProjectileVisualUpdateGroup : ComponentSystemGroup
{
}
    ...
    public partial struct ProjectileSimulationsJob : IJobEntity, IJobEntityChunkBeginEnd
    {
        ...
        void Execute(Entity entity, ref PrefabProjectile projectile, ref LocalTransform localTransform,in DynamicBuffer<WeaponShotIgnoredEntity> ignoredEntities)
        {
            if(projectile.HasHit == 0)
            {
                ...
                float3 displacment = projectile.Velocity * DeltaTime;
                ...
                if (NetworkTime.IsFirstTimeFullyPredictingTick)
                {
                    ...
                    RaycastInput raycastInput = new RaycastInput
                    {
                        ...
                        End = localTransform.Position + displacment,
                        Filter = CollisionFilter.Default,
                    };
                    PhysicsWorld.CastRay(raycastInput, ref m_Hits);
                    if(WeaponUtilities.GetClosestValiWeaponRaycastHit(in m_Hits, in ignoredEntities, out RaycastHit closestValidHit))
                    {
                        displacment *= closestValidHit.Fraction;
                ...
                localTransform.Position += displacment;
            }
...
public partial struct FinalizeWeaponLastVisualTotalProjectilesCountSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        var finalizeWeaponLastVisualTotalProjecilesCountJob = new FinalizeWeaponLastVisualTotalProjectilesCountJob();
        state.Dependency = finalizeWeaponLastVisualTotalProjecilesCountJob.Schedule(state.Dependency);
    }
    [BurstCompile]
...
public partial struct RaycastWeaponProjectileVisualsSystem : ISystem
{
    ...
    public partial struct RaycastWeaponProjectileVisualsJob : IJobEntity, IJobEntityChunkBeginEnd
    {
        ...
        void Execute(
            ...
            ref DynamicBuffer<RaycastWeaponVisualProcjetileEvent> visualProjectileEvents,
            ...
            in WeaponShotSimulationOriginOverride shotSimulationOriginOverride)
        {
            ...
            if (raycastWeapon.VisualSyncMode == RaycastWeaponVisualSyncMode.BandwithEfficient)
            {
                if (baseWeapon.LastVisualTotalProjectilesCount < baseWeapon.TotalProjectilesCount)
                {
                            ...
                            out RaycastProjectile raycastProjectile))
                    {
                        ...
                        for (var i = baseWeapon.LastVisualTotalProjectilesCount; i < baseWeapon.TotalProjectilesCount; i++)
                        {
                            ...
                            visualProjectileEvents.Add(new RaycastWeaponVisualProcjetileEvent
                            {
            ...
            if (visualProjectileEvents.Length > 0)
            {
                ...
                {
                    RaycastWeaponVisualProcjetileEvent visualProjectileEvent = visualProjectileEvents[i];
                    ...
                    if (visualProjectileEvent.Tick > raycastWeapon.LastProcessedProjectileVisualEventTick)
                    {
                        ...
                        Entity shotVisualsEntity = Ecb.Instantiate(raycastWeapon.ProjecttilePrefab);
...
public partial struct PrefabProjectileVisualsSystem : ISystem
{
    ...
    public partial struct PrefabProjectileVisualsJob : IJobEntity
    {
        ...
        {
            float3 visualOffset = math.lerp(projectile.VisualOffset, float3.zero, math.saturate(projectile.LifetimeCounter / projectile.VisualOffsettCorrectionDuration));
