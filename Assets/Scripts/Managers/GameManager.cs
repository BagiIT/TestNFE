...
public class GameManager : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (gameInput.Debug.ReturnToMainMenu.WasPerformedThisFrame())
        {
            if (GameSettings.Instance.GameState != GlobalGameState.MainMenu) ReturnToMainMenuAsync();
            else QuitAsync();
        }

        if (gameInput.Debug.StartClientServer.WasPerformedThisFrame()) StartGameAsync(CreationType.Create);
        if (gameInput.Debug.StartClient.WasPerformedThisFrame()) StartGameAsync(CreationType.Join);
    }
    ...
    async Task StartGameAsync(CreationType creationType, CancellationToken cancellationToken)
    {
        ...
        switch (creationType)
        {
            ...
            case (CreationType.QuickJoin):
                {
                    m_GameConnection = await GameConnection.JoinOrCreateMatchmmakerGameAsync(cancellationToken);
                    break;
                }
        }
